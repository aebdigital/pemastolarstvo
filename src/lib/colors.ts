...
export function getSlovakDisplayName(baseFileName: string): string {
  // strip extension
  const clean = baseFileName.replace(/\.(png|jpg)/, '');
  return slovakNames[clean] || clean;
}

export function getColorsForType(doorType: 'ramove' | 'sendvicove'): DoorColor[] {
  const available = variantAvailableColors[doorType === 'sendvicove' ? 'sendvicove' : 'ramove'];
  return available.map(file => {
    const swatchFile = unifiedColorMapping[file];
    return {
      code: file,
      name: getSlovakDisplayName(file),
